...
<table>
    {{- range $result := . }}
                ...
                {{- range $vuln := $result.Vulnerabilities }}
                    {{ if (eq (escapeXML $vuln.Vulnerability.Severity) $severity_level) }}
                        <tr class="severity-{{ escapeXML $vuln.Vulnerability.Severity }}">
                            ...
                            <td>{{ escapeXML $vuln.Description }}</td>
        ...
        {{- else }}
            ...
            {{- range $severity_level := list "CRITICAL" "HIGH" "MEDIUM" "LOW" "UNKNOWN"}}
                {{- range $missconf := $result.Misconfigurations }}
                    {{ if (eq (escapeXML $missconf.Severity) $severity_level) }}
                        <tr class="severity-{{ escapeXML $missconf.Severity }}">
                            <td class="misconf-type">{{ escapeXML $missconf.Type }}</td>
                            <td>{{ escapeXML $missconf.ID }}</td>
                            <td class="misconf-check">{{ escapeXML $missconf.Title }}</td>
                            <td class="severity">{{ escapeXML $missconf.Severity }}</td>
                            <td class="link" data-more-links="off" style="white-space:normal;">
                                {{ escapeXML $missconf.Message }}
                                <br>
                                    <a href={{ escapeXML $missconf.PrimaryURL | printf "%q" }}>{{ escapeXML $missconf.PrimaryURL }}</a>
                                </br>
                            </td>
                        </tr>
                    {{- end}}
